...
const destinationLinks = document.querySelectorAll(".box");

// Menambahkan event listener untuk setiap elemen
destinationLinks.forEach((link) => {
  link.addEventListener("click", (e) => {
    // Mencegah pergi ke halaman destination secara default
    e.preventDefault();

    // Mendapatkan id destination dari id elemen yang diklik
    const destinationId = link.id;

    // Mengarahkan pengguna ke halaman destination yang sesuai dengan id destination
    window.location.href = `destination.html#${destinationId}`;
  });
});
